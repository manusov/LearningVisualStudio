/*
Class for sine calculation speed test.
*/
...
int Task_03::execute(int argc, char** argv)
{
	char msg[APPCONST::MAX_TEXT_STRING];
	double dummySum = 0;
	std::cout << std::endl;
	int localStatus = scenarioSingleThread(dummySum);
	snprintf(msg, APPCONST::MAX_TEXT_STRING, "Dummy sum = %.3f.\r\n", dummySum);
	AppLib::writeColor(msg, APPCONST::VALUE_COLOR);
	if (localStatus == 0)
	{
		AppLib::writeColor("\r\n", APPCONST::TABLE_COLOR);
		localStatus = scenarioMultiThread(dummySum);
		snprintf(msg, APPCONST::MAX_TEXT_STRING, "Dummy sum = %.3f.\r\n", dummySum);
		AppLib::writeColor(msg, APPCONST::VALUE_COLOR);
	}
	return localStatus;
...
int Task_03::scenarioSingleThread(double& dummySum)
{
	char msg[APPCONST::MAX_TEXT_STRING];
	int exitCode = 0;
	dummySum = 0;
...
int Task_03::scenarioMultiThread(double& dummySum)
{
	char msg[APPCONST::MAX_TEXT_STRING];
	int exitCode = 0;
	dummySum = 0;
...
double Task_03::sumHelper(double* p, size_t count)
{
	double sum = 0;
...
void Task_03::threadFunctionAsm(void* ptr)
{
	DWORD64 deltaTsc = 0;
	calcSine(ptr, COUNT, REPEATS, deltaTsc, NUM);
}
...
void Task_03::threadFunctionCpp(void* ptr)
{
	double a = NUM;
	for (int j = 0; j < REPEATS; j++)
	{
...
void Task_03::threadFunctionAvx512(void* ptr)
{
	__m512d x = { NUM, 2 * NUM, 3 * NUM, 4 * NUM, 5 * NUM, 6 * NUM, 7 * NUM, 8 * NUM };
	__m512d dx = { 8 * NUM, 8 * NUM, 8 * NUM, 8 * NUM, 8 * NUM, 8 * NUM, 8 * NUM, 8 * NUM };
	for (int j = 0; j < REPEATS; j++)
