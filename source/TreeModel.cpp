...
UINT TreeModel::GetIconIdByIndex(int index)
{
	UINT id = 0;
	if (index < ICON_COUNT)
	{
...
HICON TreeModel::GetIconHandleByIndex(int index)
{
	HICON hIcon = NULL;
	if (index < ICON_COUNT)
	{
...
LPCSTR TreeModel::GetIconNameByIndex(int index)
{
	LPCSTR name = NULL;
	if (index < ICON_COUNT)
	{
...
void TreeModel::SetBase(POINT b)
{
	base = b;
}
PTREECHILDS TreeModel::getNodeChilds()
{
	return nodeChilds;
}
int TreeModel::getNodeChildsCount()
{
	return NODE_CHILDS_COUNT;
}
BOOL TreeModel::getInitStatus()
{
	return initStatus;
}

const char* TreeModel::ICON_NAMES[] =
{
	"Application icon",
...
TREECHILDS TreeModel::nodeChilds[] = {
...
int TreeModel::NODE_CHILDS_COUNT = sizeof(nodeChilds) / sizeof(TREECHILDS);
// Resources initialization status
BOOL TreeModel::initStatus = FALSE;
