...
int main(int argc, char** argv)
{
    ...
    if (backOutPtr)
    {
        if (AppLib::initConsole(screenMode, fileMode, backOutPtr, APPCONST::MAX_OUT_FILE_SIZE))
        {
            ...
            STATUS_CODES opStatus;
            switch (pCommandLineParms->optionModel)
            {
            case SINGLE_THREAD:
                opStatus = TaskWorkSingleThread::checkThreads(pCommandLineParms);
                break;
            case MULTI_THREAD:
                opStatus = TaskWorkMultiThread::checkThreads(pCommandLineParms);
                break;
            default:
