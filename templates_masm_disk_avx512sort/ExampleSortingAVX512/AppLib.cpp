...
void AppLib::writeParmAndValue(const char* parmName, const char* parmValue, int nameWidth, WORD valueColor)
{
    char temp[APPCONST::MAX_TEXT_STRING];
...
void AppLib::calculateStatistics(std::vector<double> &data, double& min, double& max, double& average, double& median)
{
    ...
    if (n)
    {
        ...
        double sum = std::accumulate(data.begin(), data.end(), double(0));  // BUG FIXED: double(0). Otherwise integer only addition.
        min = data[0];
        max = data[n - 1];
        average = sum / n;
...
bool AppLib::gScreenMode = false;
bool AppLib::gFileMode = false;
char* AppLib::gOutPointer = nullptr;
size_t AppLib::gMaxOutSize = 0;
size_t AppLib::gCurrentOutSize = 0;
HANDLE AppLib::gHstdout = nullptr;
CONSOLE_SCREEN_BUFFER_INFO AppLib::csbi{ 0 };
